...
class ClientUpdateAck(db.Model):
    """Отметка, что конкретный number уже выполнил событие ClientUpdate.id для своей пятёрки."""
    id = db.Column(db.Integer, primary_key=True)
    range_id = db.Column(db.Integer, db.ForeignKey("number_range.id"), nullable=False)
    group_index = db.Column(db.Integer, nullable=False)
    number = db.Column(db.Integer, nullable=False)
    last_ack_id = db.Column(db.Integer, nullable=False, default=0)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow)

    __table_args__ = (
        db.UniqueConstraint("range_id", "group_index", "number", name="uq_client_update_ack"),
    )
...
def api_client_update_check():
    ...
    if not row:
        ...
        # Если этот бот уже подтвердил (ACK) событие — больше его не отдаём
    ack = (
        ClientUpdateAck.query
        .filter_by(range_id=rng.id, group_index=group_index, number=number)
        .first()
    )
    last_done = int(ack.last_ack_id) if ack and ack.last_ack_id else 0
    if int(row.id) <= last_done:
        return jsonify({"ok": True, "id": None, "group_index": group_index})
...
def api_client_update_ack():
    """Клиент подтверждает, что обновился по событию ClientUpdate.id (защита от повторов после перезапуска)."""
    data = request.get_json(force=True, silent=True) or {}
    number = data.get("number")
    event_id = data.get("id")

    if number is None or event_id is None:
    ...
    row = (
        ClientUpdateAck.query
        .filter_by(range_id=rng.id, group_index=group_index, number=number_int)
        .first()
    )
    if not row:
        row = ClientUpdateAck(
    ...
    row.last_ack_id = max(int(row.last_ack_id or 0), event_id_int)
    row.updated_at = datetime.utcnow()
    db.session.commit()

    return jsonify({"ok": True, "group_index": group_index, "last_ack_id": row.last_ack_id})
